  ...
  if number < 0:
    for i in range(1,int(math.sqrt(-number)+1)):
      if -number % i == 0:
        ...
        list_of_factors.append(-i) # add negative factor to list
        list_of_factors.append((-number) // i) # add factor to list if number is divisible by i
        list_of_factors.append(-((-number) // i)) # add negative factor to list
